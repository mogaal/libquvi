...
=item B<-s, --no-shortened>

Do not "decompress" shortened URLs before using them.
...
Test all of the built-in (category QUVIPROTO_HTTP) URLs.

=item B<--dump>

Dump video details while with C<--test-all>.

=item B<-t, --test> I<arg>

Match I<arg> to built-in (category QUVIPROTO_HTTP) test URLs, e.g.:
...
Query for video format I<arg>. Run quvi with C<--support> to get a list
...
Check if URL is supported. Note that some of the shortened URLs will
fail with this. quvi exit status will be the last exit status of the
last checked URL. See also L</EXIT STATUS>.
...
Compile quvi with C<-g>, refer to C<gcc(1)> documentation for the
details.
